...
1. 把 `恶名狩猎` 排在 `体力刷本` 前面执行。
2. `恶名狩猎` 打的是每周 3 次免费的 `周期挑战`，不消耗电量。
3. 体力计划里的 `恶名狩猎 深度追猎` 要消耗电量，并且要等 `周期挑战` 的免费次数用完才能跑；还有免费次数时，脚本会临时跳过这条计划。

让 `恶名狩猎` 先把免费次数清掉，体力计划里的 `深度追猎` 就不会被跳过，相当于把深度追猎从「周期挑战剩余次数」的限制里解放出来。

::: tip
（真）典型使用方式：不打深度追猎的话，体力计划里不放这条计划就行，`恶名狩猎` 单独跑也能拿满每周奖励。
:::
