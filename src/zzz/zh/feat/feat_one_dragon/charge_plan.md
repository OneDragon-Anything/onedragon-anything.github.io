...
其中 `合成电池` 是个特殊计划：不会传送到副本或进入战斗，而是从快捷手册资源栏进入以太电池合成，所以没有副本类型、预备编队和自动战斗配置，只需设置 `已运行次数 / 计划次数`
...
4. 恢复电量
   ...
   - 体力计划运行前会预读快捷手册资源栏：`实战模拟室` 为卡片数量 ×20 点；`专业挑战室` 为 40 点；`区域巡防`、`恶名狩猎 深度追猎` 和 `合成电池` 为 60 点。
   - 当前电量+所选恢复来源足够才会继续进本，恢复电量会在副本内点 `下一步` 后，或战斗结束后点 `再来一次` 触发恢复弹窗时执行。
