...
- `无`：适用于可以直接访问所选下载源，或代理已经配置在路由器、系统网络中的情况。
   - 例如：你当前的网络可以直连 GitHub、CNB 或 Gitee，或者你使用了 VPN/代理软件的 TUN/全局模式。
- `个人代理`：使用本机代理软件的 HTTP 监听地址，该代理会用于安装器的网络请求，请确认代理软件正在运行且监听地址填写正确。
   - 例如：代理软件提供了本地监听端口，则可填写个人代理（比如 `http://127.0.0.1:7890`）。
- `GitHub 代理` 只用于 GitHub 相关的代码和文件下载，不是所有网络请求的通用代理。当免费代理地址失效时，需要更换可用方式。
:::
...
适合需要修改代码或参与开发的用户，普通用户优先使用集成启动器或原始启动器（安装器）。
...
2. 按照[安装路径要求](#安装路径要求)，创建并进入用于存放仓库的父目录
...
1. 运行代码
   - 更新方式：
     ...
     - <font color="red">进入左侧导航的 `资源管理`，点击 `代码同步` 可手动更新</font>
   ...
   - 在 `资源管理` 页的 `代码同步` 卡片中点击 `设置`，可以切换代码源、分支或开启 `强制更新`；`自动` 代码源会优先使用上次成功的源，失败或超时后继续尝试其他源
2. 启动器
   - 在 `资源管理` 的 `启动器` 卡片中选择 `原始启动器` 或 `集成启动器`，再选择稳定版或测试版进行更新
   - 手动更新时，原始启动器使用 `Launcher.zip`，集成启动器使用 `RuntimeLauncher.zip`
3. 识别模型
   - **一条龙依赖图像识别，及时更新识别模型非常重要**
   - 在 `资源管理` 中更新 OCR、闪光识别、空洞格子识别和迷失之地识别模型
